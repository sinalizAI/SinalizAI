...
class EditScreen(BaseScreen):
    ...
    def go_to_change_password(self):
        ...
        try:
            ...
            forgot_screen.original_edit_previous = original_previous
    ...
    def save_changes(self):
        ...
        if name_changed:
            ...
            if status != 200 or not isinstance(response, dict) or not response.get('success'):
                ...
                tempo = end - start
                print(f" Tempo de resposta (alterar nome): {tempo:.3f}s")
                from utils.benchmark_logger import log_benchmark
                log_benchmark('alterar_nome', tempo, {'novo_nome': new_name})
                return
